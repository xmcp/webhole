...
class ControlBar extends PureComponent {
    ...
    on_keypress(event) {
        if(event.key==='Enter') {
            ...
            const mode=/^[#$]\d+$/.test(this.state.search_text) ? 'single' : 'search';
        ...
        return (
            <TokenCtx.Consumer>{({value: token})=>(
                <div className="control-bar">
                    ...
                    <input className="control-search" value={this.state.search_text} placeholder="搜索 或 #ID"
                    ...
                    {!!token &&
                        <a className="no-underline control-btn" onClick={()=>{
                            this.props.show_sidebar(
                                '发表树洞',
                                <PostForm token={token} pid={null} on_complete={()=>{
