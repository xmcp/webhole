...
const BUILTIN_IMGS={
    'static/bg/default.png': '_BRAND_DEFAULT_BGIMG_NAME',
};

const DEFAULT_CONFIG={
    background_img: 'static/bg/default.jpg',
...
export function load_config() {
    ...
    let loaded_config;
    try {
        loaded_config=JSON.parse(localStorage['WEBHOLE_CONFIG']||'{}');
        // config migration can be added here
    } catch(e) {
...
export function save_config() {
    localStorage['WEBHOLE_CONFIG']=JSON.stringify(window.config);
    load_config();
}
        ...
        return (
            <div>
                ...
                <div className="box">
                    ...
                    <hr />
                    <ConfigSwitch callback={this.save_changes_bound} id="back_shortcut" name="快速返回"
                                  description="短暂按住 Esc 键或重压屏幕（3D Touch）可以快速返回或者刷新树洞"
                    />
                    <hr />
                    <ConfigSwitch callback={this.save_changes_bound} id="blur_effect" name="使用模糊效果"
                                  description="对界面元素采用模糊背景效果，更好看，但在某些设备上可能导致卡顿问题"
                    />
                    <hr />
